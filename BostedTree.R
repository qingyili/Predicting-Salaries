...
train<-read.csv("train_train.csv", header=T)
test<-read.csv("train_test_set.csv", header=T)
validation<-read.csv("train_validation.csv", header = T)
...
final_prediction_test0 = read.csv("test.csv", header=T)
...
write.csv(data.frame(CASEID, morethan60kyr), file = "final_100.csv", row.names = F)
morethan60_prev <- read.csv("final_just_2_columns.csv")[,2]
...
zz <- file("final_100_wCASEID.txt", open = "wt")
sink(zz)
...
write.csv(data.frame(CASEID, morethan60kyr_wCASEID), file = "final_100_wCASEID.csv", row.names = F)
morethan60_prev_noCASEID <- read.csv("final_100.csv")[,2]
